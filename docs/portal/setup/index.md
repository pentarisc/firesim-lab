...
The host side is short by design: firesim-lab ships its whole toolchain in a container image, so the host needs little more than a container runtime. The AWS side is only relevant for the FPGA path (`fslab build fpga` / `fslab sim fpga`) and can be deferred until your design already works in desktop metasimulation.

Work through the section in this order:

1. {doc}`host-prerequisites` — the checklist for the host machine: a container runtime, curl, supported platforms, and recommended hardware. **Everyone needs this.**
